...
class LoadBatchData(object):
    def __init__(self, config, split_flag, split_list=None):
        ...
        self.start_index = np.min(self.positive_eids)
    ...
    def get_data(self, dataset_name, split_flag, split_list):
        ...
        test_time = list(np.quantile(graph_df.ts, [split_list[0]]))
        ...
        train_mask = np.where(timestamps <= test_time)[0]
    ...
    def getBatchData(self, batch_num):
        batch_size = self.config.batch_size
        max_batc_num = math.ceil(len(self.positive_eids) / batch_size)
        e_s = batch_size * batch_num + self.start_index
        e_e = min(batch_size * (batch_num + 1), len(self.positive_eids)) + self.start_index
        edge_idxs = self.full_data.edge_idxs[e_s: e_e]
        sources = self.full_data.sources[e_s: e_e]
        destinations = self.full_data.destinations[e_s, e_e]
        timestamps = self.full_data.timestamps[e_s, e_e]
        labels = self.full_data.labels[e_s, e_e]
        for i in range(e_e - e_s):

            src_neigh_edge, src_neigh_time, src_neigh_idx = self.ngh_finder.get_temporal_neighbor_all(sources, timestamps, self.config.n_layer, self.config.n_neighbors)
