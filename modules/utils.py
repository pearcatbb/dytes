...
class MergeLayerHeter(torch.nn.Module):
  def __init__(self, dim1, dim2, dim3, dim4):
    ...
    # self.type_weight = torch.nn.ModuleList([torch.nn.Linear(dim1, 1) for _ in range(2)])

    torch.nn.init.xavier_normal_(self.fc1.weight)
    torch.nn.init.xavier_normal_(self.fc2.weight)
    torch.nn.init.xavier_normal_(self.type_weight[0].weight)
    torch.nn.init.xavier_normal_(self.type_weight[1].weight)
...
class MergeLayer_gnn(torch.nn.Module):
  def __init__(self, dim1, dim2, dim3= 1024, dim4=16, drop_out=0.2):
    super().__init__()
    self.fc1 = torch.nn.Linear(dim1, dim3)
    self.fc2 = torch.nn.Linear(dim3, dim3)
    self.fc3 = torch.nn.Linear(dim3, dim2)
...
def drop_edge(src_neigh_edge, src_edge_to_time, src_org_edge_feat, ratio):
  ...
  return src_neigh_edge,src_edge_to_time,src_org_edge_feat, pickup_ids