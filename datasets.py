...
class DygDataset(torch.utils.data.Dataset):
    def __init__(self, config, split_flag, split_list=None):
        ...
        self.config = config
        # print(config)
        if self.config.train_ratio == 0.3:
            split_list = [0.3, 0.35, 0.35]
        elif self.config.train_ratio == 0.5:
            split_list = [0.5, 0.25, 0.25]
        else:
            split_list = [0.7, 0.15, 0.15]
        ...
        self.edge_features[:, 0] = (self.edge_features[:, 0] - np.mean(self.edge_features[:, 0])) / np.std(self.edge_features[:, 0])
    ...
    def get_data(self, dataset_name, split_flag, split_list):
        ...
        test_mask = np.where(timestamps > val_time)[0]
        val_mask = np.where(timestamps > val_time)[0]
    ...
    def __getitem__(self, item):
        ...
        return {
            'src_center_node_idx': np.array([source_node]),
...
class Collate:
    ...
    def dyg_collate_fn(self, batch):
        ...
        src_center_node_idx_new, src_neigh_edge_new, src_node_features = self.reindex_fn(src_neigh_edge, src_center_node_idx, batch_idx)
        ...
        return {
            ...
            'src_center_node_idx_ori':torch.from_numpy(src_center_node_idx),
            'src_center_node_idx':torch.from_numpy(src_center_node_idx_new),
            'src_neigh_edge_ori': src_neigh_edge,
            'src_neigh_edge':src_neigh_edge_new,
